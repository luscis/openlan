...
setup_sw2() {
  ...
  assert_cmd docker exec $name openlan network --name example output add \
    --remote 172.249.0.241 --protocol tcp \
    --secret uplink@example:123456 --crypt aes-128:$crypt_secret

  # Return path for VPN subnet when redirected through sw2.
  assert_cmd docker exec $name openlan network --name example route add \
    --prefix 10.97.0.0/24 --nexthop 192.53.0.1
}
...
setup_openvpn_client() {
  ...
  assert_cmd docker exec $name openlan network --name example route add --prefix $sw2_vip/32 --nexthop 192.53.0.2
  assert_cmd docker exec $name openlan network --name example openvpn add \
    --listen :1194 --protocol tcp --subnet 10.97.0.0/24 --dns 8.8.8.8
...
test_redirect() {
  ...
  assert_unmatch 5 "docker exec $vpn1_name wget -qO- -T 3 -t 1 http://$sw1_vip:8081" "sw1-vip"
}
