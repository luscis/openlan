...
show_description() {
  echo "verify bgp peer establishment and prefix filter persistence"
}

show_topology_summary() {
  cat <<'EOF'
sw1 100.100.0.241 / AS65101 <---- BGP ----> sw2 100.100.0.242 / AS65102 | svc 192.54.0.1 svc 192.54.0.2
EOF
}
...
# Topology:
# - Diagram:
#       sw1 100.100.0.241 / AS65101  <---- BGP ---->  sw2 100.100.0.242 / AS65102
...
# - BGP design:
#   sw1 local-as 65101, router-id 100.100.0.241.
#   sw2 local-as 65102, router-id 100.100.0.242.
...
setup_sw1() {
  ...
  assert_cmd docker exec $name openlan bgp enable --router-id 100.100.0.241 --local-as 65101
  assert_cmd docker exec $name openlan bgp neighbor add --address 100.100.0.242 --remote-as 65102
}
...
setup_sw2() {
  ...
  assert_cmd docker exec $name openlan bgp enable --router-id 100.100.0.242 --local-as 65102
  assert_cmd docker exec $name openlan bgp neighbor add --address 100.100.0.241 --remote-as 65101
}

setup_prefix_filters() {
  assert_cmd docker exec $sw1_name openlan bgp advertis add --neighbor 100.100.0.242 --prefix 192.54.0.0/24
  assert_cmd docker exec $sw1_name openlan bgp receives add --neighbor 100.100.0.242 --prefix 192.55.0.0/24

  assert_cmd docker exec $sw2_name openlan bgp advertis add --neighbor 100.100.0.241 --prefix 192.55.0.0/24
  assert_cmd docker exec $sw2_name openlan bgp receives add --neighbor 100.100.0.241 --prefix 192.54.0.0/24
}
