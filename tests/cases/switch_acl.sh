...
show_description() {
  echo "verify acl add-list-save-reload-remove with vip tcp/80 and icmp"
}

show_topology_summary() {
  cat <<'EOF'
sw1 192.61.0.1 -- UDP output --> sw2 192.61.0.2 | +------ ACL checks ----------> VIP 10.254.0.12:80/ICMP
EOF
}
...
# Topology:
# - Diagram:
#       sw1 192.61.0.1  -- UDP output -->  sw2 192.61.0.2
#              |                              |
#              +------ ACL checks ----------> VIP 10.254.0.12:80/ICMP
...
setup_sw2() {
  ...
  assert_cmd docker exec $name openlan user add --name t1@example --password 123456
  assert_cmd docker exec $name openlan network --name example output add --remote 100.100.0.241 --protocol udp --secret t1@example:123456 --crypt aes-128:$crypt_secret
}
...
test_acl_add() {
  assert_match 5 "docker exec $sw1_name wget -qO- -T 3 -t 1 http://$vip_address:80" "acl-vip-80"
  assert_cmd docker exec $sw2_name openlan acl --name example rule add --srcip 192.61.0.1 --dstip $vip_address --protocol tcp --dport 80
  ...
  assert_match 5 "docker exec $sw1_name ping -c 3 $vip_address" "bytes from"
  assert_cmd docker exec $sw2_name openlan acl --name example rule add --srcip 192.61.0.1 --dstip $vip_address --protocol icmp
