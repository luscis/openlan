...
EOF
  ...
  # Add a network.
  docker exec $name openlan network --name example add --address 172.11.0.1/24
...
EOF
  ...
  # Add a network.
  docker exec $name openlan network --name example add --address 172.11.0.2/24
  # Add a output
...
ping() {
  wait "docker exec tests-sw2 ping -c 15 172.11.0.1" "bytes from" 15
}
