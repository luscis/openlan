...
func (h QosApi) Router(router *mux.Router) {
	router.HandleFunc("/api/qos/{id}", h.List).Methods("GET")
	router.HandleFunc("/api/qos/{id}", h.Add).Methods("POST")
	router.HandleFunc("/api/qos/{id}", h.Del).Methods("DELETE")
}
...
func (h QosApi) Add(w http.ResponseWriter, r *http.Request) {
	...
	if qos != nil {
		if err := worker.Qos().AddUserQos(qos.Name, qos.InSpeed, qos.OutSpeed); err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
		ResponseJson(w, true)
...
func (h QosApi) Del(w http.ResponseWriter, r *http.Request) {
	...
	if qos != nil {
		if err := worker.Qos().DelUserQos(qos.Name); err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
		ResponseJson(w, true)
